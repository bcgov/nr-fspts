...
/**
 * BC-gov WMS overlay layers + the base-map + polygon styling, ported
 * verbatim from nr-silva's OpeningsMap so the FDU map offers the exact
 * same layer catalogue and look. Overlays are served by the BC
 * Geographic Warehouse WMS endpoint (openmaps.gov.bc.ca/geo/ows); each
 * entry names the warehouse layer and the published style to request.
 */
...
// basemap beneath.
export const FDU_STYLE: PathOptions = {
  fillColor: '#4A90E2',
  color: '#2C3E50',
  weight: 2,
  opacity: 1,
